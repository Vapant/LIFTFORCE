...
# ==========================================
def main():
    print(" Запуск с зоной анализа (ROI Слева/Справа/Сверху)...")

    VIDEO_SOURCE = 'test_video_7.mp4'
    YOLO_MODEL = 'best.pt'
    OUTPUT_FILE = 'output_result_6.mp4'
    ...
    ROI_RIGHT_CUT = 0.15  # Отрезать правые 15% (оставить центр)
    ...
    try:
        ...
        observer = PlateObserver(confidence_threshold=3)
    ...
    while True:
        ...
        for result in results:
            ...
            for box, track_id in zip(boxes, track_ids):
                ...
                # --- ВАЖНО: ПЕРЕСЧЕТ КООРДИНАТ ---
                # Добавляем смещение по Y (сверху) и по X (слева)
                ...
                if track_id in observer.confirmed:
                    ...
                    cv2.putText(frame, f"ID:{track_id} {final_text}", (x1, y1 - 10),
                ...
                if frame_count % SKIP_FRAMES == 0:
                    pad = 10
                    # Вырезаем из полного кадра, используя пересчитанные координаты
    ...
    print(f" Работа завершена. Видео сохранено в {OUTPUT_FILE}")
